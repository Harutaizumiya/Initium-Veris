...
Initium-Veris 是一个基于 Turborepo 的食品库存管理 monorepo。项目包含 Web 管理端、Expo/React Native 移动端、共享 TypeScript API client 和 Django REST API，覆盖登录认证、组件级权限、商品与批次、库存操作、盘点报损、效期预警、二维码标签和扫码审计等业务流程。

> 当前 Web 管理端默认使用前端内存 demo 数据，适合快速体验界面和交互；移动端与共享 API client 默认对接 Django `/api`，后端可独立部署为生产服务。
...
- **库存看板**：汇总库存总览、效期风险、库存变化趋势和待处理事项。
- **商品与批次管理**：维护商品主数据、批次信息、存储位置、保质期和库存状态。
- **库存操作链路**：支持入库、出库、调整、撤销、报损和操作审计。
- **效期预警**：根据批次到期时间计算临期、过期和正常状态，辅助优先处理风险库存。
- **二维码标签**：为批次生成二维码凭证，支持打印标签和移动端扫码核验。
- **扫码审计**：记录扫码来源、批次状态和处理结果，保留可追溯操作记录。
- **角色与权限**：通过后端账号、角色和组件级权限控制 Web 与移动端入口。
...
- Python 3.12+
- uv
- PostgreSQL 14+，用于本地或生产 Django API
- Docker，可选，仅生产镜像部署需要
- Redis，可选，仅启用共享缓存时需要
...
如果只想预览 Web 管理端界面，可以直接启动前端。当前 Web 默认使用内存 demo 数据，不需要先启动数据库或后端：
...
如果需要运行 Django API，在 `apps/api/.env` 中配置数据库：
...
后端默认运行在 `http://127.0.0.1:8000`，健康检查接口：
...
| `pnpm dev:debug` | 以前端 debug 模式启动 Web，便于查看日志面板和控制台日志 |
...
移动端连接本地 API 时需要设置 API 地址。真机调试时请使用电脑的局域网 IP：
...
参考 [apps/web/.env.example](apps/web/.env.example)。当前 Web demo 不需要 API 地址，日志相关变量包括：
...
以 Nginx 为例，把 `/api/` 转发到本机 Django API：
...
### 4. 使用 GitHub Actions 自动部署后端

仓库内置 `.github/workflows/deploy-api.yml`，会在 push 到 `main` 或手动触发时部署后端。
...
可以部署到任意静态托管服务或 Nginx 静态目录。若后续将 Web 从 demo 模式切换为真实 API，请同步配置生产 API 地址和认证策略。
...
欢迎通过 Issue 或 Pull Request 讨论改进。提交前建议至少运行：
