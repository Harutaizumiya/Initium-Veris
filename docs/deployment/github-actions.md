...
4. 使用 `apps/api/Dockerfile` 构建 `initium-veris-api:latest`。
5. `systemctl daemon-reload`。
6. 重启 `initium-veris-api.service`。
7. 检查 systemd 服务处于 active。
8. 最多等待 20 秒检查 `http://127.0.0.1:8000/api/ping` 返回 `pong`。
